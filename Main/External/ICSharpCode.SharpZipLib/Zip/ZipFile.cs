...
//		}
		...
		/// <summary>
        /// Add a file to the archive under a different name.
        /// </summary>
        /// <param name="fileName">The name of the file to add.</param>
        /// <param name="newName">The new name of the file.</param>
        public void Add(string fileName, string newName)
        {
            if ( fileName == null ) {
                throw new ArgumentNullException("fileName");
            }
            ...
            CheckUpdating();
            Add(new StaticDiskDataSource(fileName), GetTransformedFileName(newName), CompressionMethod.Deflated);
        }
...
*/
			...
			try {
				...
				foreach ( ZipUpdate update in updates_ ) {
					...
					if ( (update.CrcPatchOffset > 0) && (update.OutEntry.CompressedSize > 0) ) {
						...
						workFile.WriteLEUint((uint)update.OutEntry.Crc);
					}
		...
		uint ReadLEUint()
		{
			return (uint)ReadLEUshort() | (uint)ReadLEUshort() << 16;
		}
